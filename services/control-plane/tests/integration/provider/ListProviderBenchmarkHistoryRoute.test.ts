...
describe("provider benchmark history route", () => {
  let pool: Pool;
  ...
  it("lists benchmark history newest first for a provider node", async () => {
    ...
    const app = buildApp({
      ...
      issueOrganizationInvitationUseCase:
        new IssueOrganizationInvitationUseCase(
          ...
          auditLog,
          () => new Date("2026-03-09T18:00:00.000Z"),
          () => "unused-provider-history-token-0001"
        ),
      ...
      issueOrganizationApiKeyUseCase: new IssueOrganizationApiKeyUseCase(
        ...
        () => "csk_provider_history_secret_000000"
      ),
    ...
    const createOrganizationResponse = await app.inject({
      ...
      payload: {
        name: "Acme Provider Benchmark History",
        slug: "acme-provider-benchmark-history",
        founder: {
          email: "provider-history-owner@example.com",
          displayName: "Provider History Owner"
        },
    ...
    const nodeResponse = await app.inject({
      ...
      payload: {
        machineId: "node-machine-history-0001",
        label: "Benchmark History Node",
        runtime: "linux",
        region: "eu-central-1",
        hostname: "node-history-01.internal",
        inventory: {
